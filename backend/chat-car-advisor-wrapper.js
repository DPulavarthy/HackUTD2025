...
wss.on('connection', function connection(ws) {
  ...
  ws.on('message', async function message(data) {
    ...
    main(data);
    // ws.send("/Users/kurasad/Documents/AIAPI/car-recommendations.html") // CHANGE TO SPECIFIC USER PATH!
  });
});
...
async function main(data) {
  ...
  const userPrompt = 
    `${systemMsg}\n\n` +
    `User's desired vehicle specifications: ${data}\n\n` +
  ...
  if (!response.ok) {
    console.error('Error from Gemini API:', JSON.stringify(res, null, 2));
  ...
  const specWords = data.toString().toLowerCase().split(/[\s,]+/).filter(w => w && w !== 'and' && w !== 'or' && w !== 'the' && w !== 'a');
  ...
  const dataScript = `
    <script>
        ...
        window.USER_SPEC = ${JSON.stringify(data)};
    </script>
`;
  ...
  await fs.writeFile('../frontend/public/car-recommendations.html', htmlTemplate, 'utf-8');
